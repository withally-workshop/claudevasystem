...
| 18 | Krave — Price Reply Auto-Resubmit | `nzFTk4e9NRi6Jk9r` | Active | Every 30min, 8–19h PHT Mon–Fri (`*/30 8-19 * * 1-5`) + `POST /webhook/krave-price-reply-resubmit` | Detect bot "price missing" threads with unprocessed amount replies in #payments-invoices-updates, parse receipt + amount, resubmit to intake webhook automatically. **Throttled 2026-06-10** from 24/7 `*/10` (was ~4,300 execs/mo, blew the 2,500 cap) → ~530/mo. TARGET: drop schedule, go fully event-driven via `message.groups` (private channel) once Slack app subscribes + Approval Reply Trigger fans out. |
