...
return [{ json: {
  refillMin: sgtDate(105),  // 105 days ago — outer edge of refill window
  refillMax: sgtDate(75),   // 75 days ago — inner edge of refill window
  upsellMin: sgtDate(120),  // 120 days ago — showerhead buyers who may not have filters yet
  upsellMax: sgtDate(14),   // 14 days ago — exclude very recent (still in decision window)
  weekOf: now.toLocaleDateString('en-SG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'Asia/Singapore' }),
} }];
`.trim();

// n4: Combine both Shopify responses and extract refill-due + upsell-gap lists.
// $json = upsell candidate orders (from Fetch Showerhead Orders).
// $('Fetch Refill Due Orders').item.json = filter orders 75-105 days old.
...
const upsellOrders = ($json.orders || []);
const dates = $('Build Date Ranges').item.json;

// Refill due: orders that contain at least one filter SKU
const refillDue = refillOrders
  ...
  .map(o => {
    ...
    return {
      order: o.name,
      email: o.email || o.contact_email || 'no email',
      items: filterItems.map(li => li.title + ' x' + li.quantity).join(', '),
      days_ago: daysSince,
...
// Upsell gap: showerhead orders that had NO filter SKU in the same order
const upsellGap = upsellOrders
  .filter(o => {
    const items = o.line_items || [];
    const hasShowerhead = items.some(li => SHOWERHEAD_SKUS.has(li.sku));
    const hasFilter = items.some(li => FILTER_SKUS.has(li.sku));
    return hasShowerhead && !hasFilter;
  })
  .map(o => {
    ...
    return {
      order: o.name,
      email: o.email || o.contact_email || 'no email',
      product: shItem ? shItem.title : 'Showerhead',
      days_ago: daysSince,
      date: o.created_at.slice(0, 10),
    };
  })
...
return [{ json: { refill_due: refillDue, upsell_gap: upsellGap, week_of: dates.weekOf } }];
`.trim();
...
Keep it tight. No preamble. List each customer on one line.`;
...
const workflow = {
  ...
  nodes: [
    ...
    { id: 'n1', name: 'Schedule Trigger', type: 'n8n-nodes-base.scheduleTrigger', typeVersion: 1.2, position: [240, 300],
      parameters: { rule: { interval: [{ field: 'cronExpression', expression: '0 1 * * 1' }] } } },
    ...
    // n4: Combine both responses, filter by SKU, build report data
    { id: 'n4', name: 'Build Report Data', type: 'n8n-nodes-base.code', typeVersion: 2, position: [1180, 300],
      parameters: { mode: 'runOnceForAllItems', jsCode: BUILD_REPORT_DATA_CODE } },

    // n5: Claude formats the report
    { id: 'n5', name: 'Format Report', type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [1420, 300],
    ...
    { id: 'n6', name: 'Post to Slack', type: 'n8n-nodes-base.httpRequest', typeVersion: 4.2, position: [1660, 300],
  ...
  connections: {
    ...
    'Fetch Showerhead Orders': { main: [[{ node: 'Build Report Data',     type: 'main', index: 0 }]] },
