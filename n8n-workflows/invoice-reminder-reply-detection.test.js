...
assert.match(deploySource, /range:\s*'A:Y'/, 'Expected tracker read through attribution columns');
...
assert.match(deploySource, /No Reply/, 'Expected No Reply status');
...
assert.match(deploySource, /from:\(.*john@kravemedia\.co.*\)|to:\(.*john@kravemedia\.co.*\)/s, 'Expected Gmail query constrained to John threads');
