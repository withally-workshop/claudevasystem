...
const children = [
  ...
  p('{#deliverables}', { after: 0 }),
  new Paragraph({ bullet: { level: 0 }, spacing: { after: 40 }, children: [new TextRun({ text: '{item}' })] }),
  p('{/deliverables}', { after: 0 }),
  p('{#hasPerformanceTiers}', { after: 0 }),
  p('Performance Incentives apply as follows:'),
  p('{#performanceTiers}', { after: 0 }),
  new Paragraph({ bullet: { level: 0 }, spacing: { after: 40 }, children: [new TextRun({ text: '{metric} — {target} — {fee}' })] }),
  p('{/performanceTiers}', { after: 0 }),
  p('Performance-based fees are calculated on a flat-rate basis (the rate of the tier in which the applicable month’s total falls applies to the full amount) and shall be payable only to the extent that such fees exceed the monthly base fee in the applicable month.'),
  p('{/hasPerformanceTiers}', { after: 0 }),
...
const tc = [
  ...
  { lead: '5.1 Termination for Convenience:', body: 'Either party may terminate this Agreement at any time, for any reason, with {terminationNotice} written notice.' },
