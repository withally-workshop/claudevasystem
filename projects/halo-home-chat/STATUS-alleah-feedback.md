...
- ~~Wire promos to pull from Shopify~~ — **LIVE + verified 2026-06-12** (`8894ac4` + `cce6436`). John re-approved the app install with read_discounts / read_all_orders / read_price_rules (token unchanged; app = "Halo Chat API", NOT "Halo-Home Bot"). Promos fetch **automatic discounts only** — the store has ~50 active code discounts that are ALL private influencer/comp codes (incl. 100%-off); prompt has a hard never-reveal-codes rule. Verified on prod: bot quotes live "Buy 2, get 1 free" + voucher, no code leaks. Full order history now visible (back to #1001, Sep 2024) — pre-Apr-12 refund audit possible if Noa ever revisits scope. Tip for later: renaming the discount in Shopify admin improves the bot's wording automatically.
