...
async function getActiveDiscounts() {
  ...
  try {
    ...
    return (data.data.discountNodes.edges || [])
      ...
      .filter((d) => d.title && d.status === 'ACTIVE')
      .filter((d) => !d.endsAt || new Date(d.endsAt).getTime() > now)
      .map((d) => ({
        title: d.title,
        summary: d.summary || '',
        code: null,
...
module.exports = { getProducts, getPages, getBlogArticles, getOrdersByEmail, getInventoryStatus, getAllRenderedPages, getActiveDiscounts, stripHtml };
